...
class Steering
{
    ...
    func calculate(subject: Bird, neighbours: [Bird]) -> CGPoint {
        var force = CGPointZero
    ...
    //    }
    //
    func calculateWallAvoidance(bird: Bird, neighbours : [Bird]) -> CGPoint {
        return CGPointZero
    }
    
    func accumulateForce(inout runningTotal: CGPoint, forceToAdd: CGPoint) -> Bool {
    ...
    func calculateSeparation(bird: Bird, neighbours: [Bird]) -> CGPoint {
            if !separation {
            return CGPointZero
            }
